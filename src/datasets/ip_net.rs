...
pub enum UpdateNetIp {
    Add((std::net::IpAddr, u8, LogString)),
...
impl IpNetSynDataset {
    ...
    pub fn insert(&self, ip: std::net::IpAddr, net: u8, data: LogString) {
        // Todo: improve with local cache to send retries
        let _ = self.comm.try_send(UpdateNetIp::Add((ip, net, data)));
    }
    pub fn remove(&self, ip: std::net::IpAddr, net: u8) {
        // Todo: improve with local cache to send retries
...
impl IpNetDataset {
    ...
    pub fn insert<S>(&mut self, ip: std::net::IpAddr, net: u8, data: S)
    where
        S: Into<LogString>,
    {
        match ip {
    ...
    pub fn remove(&mut self, ip : std::net::IpAddr, net : u8) {
        match ip {
            std::net::IpAddr::V4(ip) => {
                let ip : u32 = ip.into();
