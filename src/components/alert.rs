...
mod tests {
    ...
    fn example_rule_matcher(rule : &SiemRule, log: &SiemLog) -> Option<(Option<SiemAlert>, Option<SiemTask>)> {
    ...
    fn example_stateful_rule_matcher(rule : &SiemRule, log: &SiemLog) -> Option<(Option<SiemAlert>, Option<SiemTask>)> {
        let description = rule.get_template_for_log(log);
        match log.event() {
            SiemEvent::Auth(auth) => {
    ...
    lazy_static! {
        ...
        static ref EXAMPLE_STATEFUL_RULE_TEMPLATES : BTreeMap<&'static str, &'static str> = {
            let mut templates = BTreeMap::new();
            templates.insert("es", "Alert for user $domain\\$username. $number login errors in less than a minute");
            templates.insert("en", "Alert for user $domain\\$username. $number login errors in less than a minute");
            templates
        };
